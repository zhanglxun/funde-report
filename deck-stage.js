...
(() => {
  ...
  class DeckStage extends HTMLElement {
    ...
    _enableRail() {
      ...
      this._snapshotAuthorCss();
      // The desktop editor can patch a document-level <style> node without
      // reloading the deck or dispatching tweakchange. The active slide sees
      // that CSS immediately, while an already-cloned rail thumb otherwise
      // keeps the old stylesheet snapshot. Re-snapshot and re-clone only
      // materialized thumbs after the patch settles.
      this._headObserver = new MutationObserver(() => {
        clearTimeout(this._authorCssTimer);
        this._authorCssTimer = setTimeout(() => {
          this._snapshotAuthorCss();
          const cs = getComputedStyle(this);
          (this._thumbs || []).forEach((t) => {
            if (t.host) {
              this._syncThumbHostAttrs(t.host, cs);
              this._refreshThumb(t.slide);
            }
          });
        }, 180);
      });
      if (document.head) this._headObserver.observe(document.head, {
        subtree: true, childList: true, characterData: true, attributes: true,
      });
    ...
    disconnectedCallback() {
      ...
      if (this._authorCssTimer) clearTimeout(this._authorCssTimer);
    ...
    _materialize(entry) {
      ...
      // Keep IDs throughout the clone. Author styles commonly scope a
      // page-specific layout to an inner container (for example `#s058
      // .tier-stack`), not the <section> root. Each thumbnail is enclosed
      // in its own shadow root, so these IDs cannot collide with the live
      // deck or with IDs in other thumbnails, while keeping them lets the
      // scoped page styles render the same structure as the full slide.
